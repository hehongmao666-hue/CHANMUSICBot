...
async def _memory_diagnostic_snapshot():
    ...
    # Avoid gc.get_objects() / full object graph scans in production /stats.
    # Those scans allocate large temporary Python lists and can themselves
    # increase allocator high-water marks on a long-running music bot.
    object_count = -1
    top_types_text = "disabled (lightweight stats)"
    task_object_count = -1
    gc_garbage_count = len(getattr(gc, "garbage", ()))
    try:
        import asyncio
        task_total = len(asyncio.all_tasks())
        task_states = Counter(
            "done" if task.done() else "pending"
            for task in asyncio.all_tasks()
        )
        task_names = ", ".join(
            f"{(task.get_name() if hasattr(task, 'get_name') else type(task).__name__)[:80]}=1"
            for task in list(asyncio.all_tasks())[:20]
        )
    except Exception:
        task_total, task_states, task_names = -1, {}, "unavailable"
    ...
    logger.info("Python object deep scan: %s", top_types_text)
    logger.info(
